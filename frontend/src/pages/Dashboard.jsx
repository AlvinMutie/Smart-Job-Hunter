...
import { Search, MapPin, Briefcase, Sparkles, Upload, AlertTriangle, ChevronRight } from 'lucide-react';
    ...
    return (
        <div className="space-y-8 animate-fade-in">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-white">Welcome back, {user?.full_name?.split(' ')[0] || 'Hunter'} 👋</h2>
                ...
                {!user?.profile?.has_resume && (
                    <div className="px-4 py-2 bg-amber-500/10 border border-amber-500/20 rounded-xl flex items-center gap-3 text-amber-500 text-sm">
                        ...
                        <span>No CV detected in your profile. <a href="/profile-setup" className="font-bold underline">Upload now</a> for AI matching.</span>
                    </div>
                )}
            </div>
...
function JobCard({ job, onMatch, isMatching, matchResult }) {
    ...
    return (
        <div className="glass-card p-6 flex flex-col md:flex-row gap-6 relative overflow-hidden">
            {matchResult && (
                <div className="absolute top-0 right-0 px-4 py-1 bg-indigo-600/20 border-b border-l border-indigo-500/30 rounded-bl-xl text-indigo-400 font-bold text-sm">
                ...
                {matchResult && matchResult.missing_skills.length > 0 && (
                    <div className="mt-4 space-y-3">
                        <div className="p-3 bg-red-500/5 border border-red-500/10 rounded-xl">
                            <p className="text-xs font-semibold text-red-400 uppercase tracking-tighter mb-1">Missing Skills</p>
                            <div className="flex flex-wrap gap-1">
                                {matchResult.missing_skills.slice(0, 5).map(skill => (
                                    <span key={skill} className="text-xs text-slate-400 decoration-red-500/50 line-through">{skill}</span>
                                ))}
                        ...
                        {matchResult.tailoring_advice && (
                            <div className="p-3 bg-emerald-500/5 border border-emerald-500/10 rounded-xl">
                                <p className="text-xs font-semibold text-emerald-400 uppercase tracking-tighter mb-2 flex items-center gap-1">
                                    <Sparkles size={12} /> Tailoring Advice
                                </p>
                                <ul className="space-y-1">
                                    {matchResult.tailoring_advice.map((advice, i) => (
                                        <li key={i} className="text-xs text-slate-400">{advice}</li>
                                    ))}
                                </ul>
                            </div>
                        )}
            ...
            <div className="flex flex-col justify-center gap-3 min-w-[140px]">
                ...
                <button
                    ...
                    className={`px-4 py-2 rounded-xl border ${applied ? 'border-emerald-500 text-emerald-500' : 'border-slate-700 hover:border-slate-500 text-slate-300'} text-sm font-medium transition-all`}
                >
